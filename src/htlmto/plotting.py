    ...
    def plot(include_e):
        ...
        if include_e:
            output_filename = os.path.join(
                calc_dir, f"{os.path.basename(calc_dir)}_DOS.png"
            )
        else:
            output_filename = os.path.join(
                calc_dir, f"{os.path.basename(calc_dir)}_DOS_noE.png"
            )
...
def plot_band_structure(calc_dir):
    try:
        ...
        ax.set_title(
            folder_name_subscripted + " bandstructure", fontsize=20, pad=10
        )
